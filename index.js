...
function drawElements(a,b,squareLength){
  ...
  shapeContainer.y = app.stage.height / 2 + squareLength/2;
...
controlPoint.on("pointerdown", (event) => {
  ...
  const onPointerMove = (event) => {
    ...
    console.log(dx, dy);
    // const findMax = Math.max(Math.abs(dy), Math.abs(dx));
    Math.abs(dy) > Math.abs(dx) ? (findMax = dy) : (findMax = dx);
    console.log(a,b,newPosition, originalPosition);
